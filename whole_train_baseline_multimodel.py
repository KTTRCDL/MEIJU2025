...
def clean_checkpoints(checkpoint_dir, best_eval_steps):
    ...
    model_prefix_list = ['best_emo', 'best_int', 'best_F1', 'best_Eacc', 'best_Iacc', 'best_Euar', 'best_Iuar']
...
def train(opt):
    ...
    for cv in range(1, opt.total_cv + 1):
        ...
        total_steps = 0
        best_emo_F1, best_int_F1, best_F1 = 0, 0, 0
        best_Eacc, best_Iacc, best_Euar, best_Iuar = 0, 0, 0, 0
        # best_eval_steps = [-1, -1, -1]
        best_eval_steps = [-1, -1, -1, -1, -1, -1, -1]
        ...
        for epoch in range(opt.epoch_count, opt.epoch_count + niter):
            for data in tqdm(dataset_train, desc=f'Epoch:{epoch} CV:{cv} Step:{total_steps}', total=math.ceil(dataset_train_size/opt.batch_size)):
                ...
                if total_steps % opt.eval_freq == 0:
                    ...
                    if emo_acc > best_Eacc:
                        best_Eacc = emo_acc
                        model.save_networks(f'{total_steps}_best_Eacc')
                        logger.info('Saving the best emotion acc model at step {} with emo_acc {}'.format(total_steps, emo_acc))
                        best_eval_steps[3] = total_steps
                    if int_acc > best_Iacc:
                        best_Iacc = int_acc
                        model.save_networks(f'{total_steps}_best_Iacc')
                        logger.info('Saving the best intent acc model at step {} with int_acc {}'.format(total_steps, int_acc))
                        best_eval_steps[4] = total_steps
                    if emo_uar > best_Euar:
                        best_Euar = emo_uar
                        model.save_networks(f'{total_steps}_best_Euar')
                        logger.info('Saving the best emotion uar model at step {} with emo_uar {}'.format(total_steps, emo_uar))
                        best_eval_steps[5] = total_steps
                    if int_uar > best_Iuar:
                        best_Iuar = int_uar
                        model.save_networks(f'{total_steps}_best_Iuar')
                        logger.info('Saving the best intent uar model at step {} with int_uar {}'.format(total_steps, int_uar))
                        best_eval_steps[6] = total_steps
        ...
        # result_recorder.write_result_to_tsv({'emo_metric': best_emo_F1, 
        #                                      'int_metric': best_F1, 
        #                                      'joint_metric': best_metric}, cv)
        result_recorder.write_result_to_tsv({'emo_F1': best_emo_F1,
                                            'int_F1': best_int_F1,
                                            'joint_F1': best_F1,
                                            'emo_acc': best_Eacc,
                                            'int_acc': best_Iacc,
                                            'emo_uar': best_Euar,
                                            'int_uar': best_Iuar}, cv)
...
def find_and_set_load_model_path_for_next(opt):
    # load_model_prefix_2_result_record_metric_name = {'best_emo': 'emo_metric', 
    #                                                  'best_int': 'int_metric', 
    #                                                  'best_F1': 'joint_metric'}
    load_model_prefix_2_result_record_metric_name = {'best_emo': 'emo_F1', 
                                                     'best_int': 'int_F1', 
                                                     'best_F1': 'joint_F1',
                                                     'best_Eacc': 'emo_acc',
                                                     'best_Iacc': 'int_acc',
    ...
    if opt.load_model_path == 'None':
        # for pretrain -> train, we need to find the opt.load_model_prefix result of the pretrain
        prefix_result = result_df[load_model_prefix_2_result_record_metric_name[opt.load_model_prefix]][:-1]
    else:
        # for train -> predict, we need to find the best F1 result of the train
        prefix_result = result_df[load_model_prefix_2_result_record_metric_name['best_F1']][:-1]
...
if __name__ == '__main__':
    ...
    # chage predict, include opt.load_model_prefix = best_F1, best_emo, best_int, save the predict reslut separately, and combine best_emo and best_int results to be the submission.csv
