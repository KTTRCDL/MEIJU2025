...
def get_scheduler(optimizer, opt):
    ...
    """
    # modify for pretrain and train
    if opt.load_model_path != 'None':
        niter = opt.pretrain_niter
        niter_decay = opt.pretrain_niter_decay
    else:
        niter = opt.niter
    ...
    if opt.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.epoch_count - niter) / float(niter_decay + 1)
            return lr_l
...
def init_weights(net, init_type='normal', init_gain=0.02, logger=None):
...
def init_net(net, init_type='normal', init_gain=0.02, device=torch.device('cuda:0'), logger=None):
    ...
    # device = torch.device('cuda:{}'.format(device) if torch.cuda.is_available() and device != '-1' else 'cpu')
    net.to(device)
    init_weights(net, init_type, init_gain=init_gain, logger=logger)
    return net
