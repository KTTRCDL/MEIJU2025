...
def extract_one(data):
    vid, input_dir, process_type, save_dir, face_dir, hog_dir, pose_dir = data
    saveVid = vid ## for folder
    if vid.endswith('.mp4') or vid.endswith('.avi') or vid.endswith('.mkv'): saveVid = vid[:-4] # for mp4 or avi files
    ...
    if os.path.exists(face_root): return
    ...
    if process_type == 'faceDir':
        exe_path = os.path.join(config.PATH_TO_OPENFACE, 'FaceLandmarkImg')
        # commond = '%s -fdir \"%s\" -out_dir \"%s\"' % (exe_path, input_root, save_root)
        commond = 'OPENBLAS_NUM_THREADS=1 %s -fdir \"%s\" -out_dir \"%s\"' % (exe_path, input_root, save_root)
        os.system(commond)
        ...
        exe_path = os.path.join(config.PATH_TO_OPENFACE, 'FeatureExtraction')
        # commond = '%s -f \"%s\" -out_dir \"%s\"' % (exe_path, input_root, save_root)
        commond = 'OPENBLAS_NUM_THREADS=1 %s -f \"%s\" -out_dir \"%s\"' % (exe_path, input_root, save_root)
        os.system(commond)
...
def extract(input_dir, process_type, save_dir, face_dir, hog_dir, pose_dir):
    ...
    n_workers = int (multiprocessing.context._default_context.cpu_count() * 0.8)
    Pool = multiprocessing.context._default_context.Pool
    pool = Pool(processes=n_workers)

    data = [(vid, input_dir, process_type, save_dir, face_dir, hog_dir, pose_dir) for vid in vids]
    data_len = len(data)
    for i in enumerate(pool.imap_unordered(extract_one, data), 1):
        bar = progbar(i[0], data_len)
        message = f'{bar} {i[0]}/{data_len} '
