...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature_level', type=str, default='UTTERANCE', help='feature level [FRAME or UTTERANCE or FRP]')
    parser.add_argument('--gpu', type=str, default='1', help='gpu id')
    params = parser.parse_args()
    
    if torch.cuda.is_available():
        gpu_id = int(params.gpu)
        if gpu_id >= 0 and gpu_id < torch.cuda.device_count():
            os.environ["CUDA_VISIBLE_DEVICES"] = params.gpu
        else:
            raise ValueError(f"Invalid GPU ID {params.gpu}. Available GPUs: 0 to {torch.cuda.device_count() - 1}")
    else:
    ...
    for i, vid in enumerate(vids, 1):
        ...
        # ??? csv_file -> npy_file, vid -> name
        # npy_file = os.path.join(save_dir, f'{vid}.npy')
        ...
        dataset = FaceDataset(name, openface_aligened_path, transform=transform)
        if len(dataset) == 0:
        ...
        if params.feature_level == 'FRAME':
        ...
        elif params.feature_level == 'UTTERANCE':
            embeddings = np.array(embeddings).squeeze()
            if len(embeddings) == 0:
            ...
            np.save(npy_file, embeddings)
        elif params.feature_level == 'FRP':
            # 抽取中间部分embedding，前10%和后10%的embedding不要
            embeddings = np.array(embeddings).squeeze()
            if len(embeddings) == 0:
                embeddings = np.zeros((1, EMBEDDING_DIM))
            elif len(embeddings.shape) == 1:
                ...
                embeddings = embeddings[int(0.1 * len(embeddings)):int(0.9 * len(embeddings))]
            np.save(npy_file, embeddings)
