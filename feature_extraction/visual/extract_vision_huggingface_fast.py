...
from dataset import FaceDataset2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Track1_English', 
                        choices=['Track1_English', 'Track1_Mandarin', 'Track2_English', 'Track2_Mandarin'], help='input dataset')
    ...
    if params.videomae_type is None:
        ...
        save_dir_fra = os.path.join(config.PATH_TO_FEATURES[params.dataset], f'{model_name}-FRA')
        save_dir_utt = os.path.join(config.PATH_TO_FEATURES[params.dataset], f'{model_name}-UTT')
    else: 
        ...
        save_dir_fra = os.path.join(config.PATH_TO_FEATURES[params.dataset], f'sunlicai-{model_name}-FRA')
        save_dir_utt = os.path.join(config.PATH_TO_FEATURES[params.dataset], f'sunlicai-{model_name}-UTT')
    # if not os.path.exists(save_dir): os.makedirs(save_dir)
    os.makedirs(save_dir_fra, exist_ok=True)
    os.makedirs(save_dir_utt, exist_ok=True)
    ...
    if len(vids) == 0:
        print(f'find videos in {face_dir} is empty, try use transcription to find videos name')
        vids = pd.read_csv(f'{config.PATH_TO_TRANSCRIPTIONS[params.dataset]}')
        vids = vids['name'].tolist()
    EMBEDDING_DIM = -1
    print(f'Find total "{len(vids)}" videos.')
    for i, vid in tqdm(enumerate(vids, 1), total=len(vids)):
        # !!! modify name(vid)
        name = vid[:-4] if vid.endswith('.mp4') or vid.endswith('.avi') or vid.endswith('.mkv') else vid
        save_file_fra = os.path.join(save_dir_fra, f'{name}.npy')
        save_file_utt = os.path.join(save_dir_utt, f'{name}.npy')
        # 处理过的直接跳过
        if os.path.exists(save_file_fra) and os.path.exists(save_file_utt): continue
        ...
        with torch.no_grad():
            # !!!!!!!!!!!! add openface_aligened_path for load frame data, not face_dir
            # face_dir = os.path.join(config.PATH_TO_ALIGNED_FACE[params.dataset], vid)
            openface_aligened_path = os.path.join(config.PATH_TO_OPENFACE_ALL[params.dataset], name)
            dataset = FaceDataset2(name, openface_aligened_path)
            # frames = func_read_frames(face_dir, vid)
            frames = [frame for frame, _ in dataset]
        ...
        # save into npy
        # save_file = os.path.join(save_dir, f'{name}.npy')
        embeddings = np.array(embeddings).squeeze()
        # save frame-level features
        if len(embeddings) == 0:
            embeddings_fra = np.zeros((1, EMBEDDING_DIM))
        elif len(embeddings.shape) == 1:
            embeddings_fra = embeddings[np.newaxis, :]
        else:
            embeddings_fra = embeddings
        np.save(save_file_fra, embeddings_fra)
        # save utterance-level features
        if len(embeddings) == 0:
            embeddings_utt = np.zeros((EMBEDDING_DIM, ))
        elif len(embeddings.shape) == 2:
            embeddings_utt = np.mean(embeddings, axis=0)
        else:
            embeddings_utt = embeddings
        np.save(save_file_utt, embeddings_utt)
